...
namespace CabinetData.Entities
{
	...
	public partial class ModuleDap : BaseDap
	{
		public ModuleDap()
		{
		}

		public ModuleDap(IDbConnection connection)
		{
			Connection = connection;
		}

		public ModuleDap(IDbTransaction transaction)
		{
			Transaction = transaction;
			Connection = transaction.Connection;
		}

		public ModuleDap(BaseDap dapProvider)
		{
			Transaction = dapProvider.Transaction;
			Connection = dapProvider.Connection;
		}

		public List<Module> GetTop(int count)
		{
			return Query<Module>(string.Format("SELECT TOP {0} * FROM {1}", count, SqlTableName)).ToList();
		}

		public Module GetByID(Int32 ID)
		{
			return Query<Module>(SqlSelectCommand + " WHERE ID=@ID", new { ID = ID }).FirstOrDefault();
		}
		...
		public void Insert(IEnumerable<Module> models)
		{
			Execute(SqlInsertCommand, models);
		}
		...
		public void Update(IEnumerable<Module> models)
		{
			Execute(SqlUpdateCommand, models);
		}
		...
		public const string SqlTableName = "Module";
		public const string SqlSelectCommand = "SELECT * FROM " + SqlTableName;
		public const string SqlInsertCommand = "INSERT INTO " + SqlTableName + " (ParentID , Name , Desciptiong) VALUES (@ParentID , @Name , @Desciptiong) ";
		public const string SqlUpdateCommand = "UPDATE " + SqlTableName + " SET ParentID=@ParentID , Name=@Name , Desciptiong=@Desciptiong WHERE ID=@ID";
		public const string SqlDeleteCommand = "DELETE FROM " + SqlTableName + " WHERE ID=@ID";
	...
	public partial class RoleDap : BaseDap
	{
		...
		public RoleDap(IDbConnection connection)
		{
			Connection = connection;
		}

		public RoleDap(IDbTransaction transaction)
		{
			Transaction = transaction;
			Connection = transaction.Connection;
		}

		public RoleDap(BaseDap dapProvider)
		{
			Transaction = dapProvider.Transaction;
			Connection = dapProvider.Connection;
		}
		...
		{
			return Query<Role>(string.Format("SELECT TOP {0} * FROM {1}", count, SqlTableName)).ToList();
		}
		...
		public const string SqlTableName = "Role";
		public const string SqlSelectCommand = "SELECT * FROM " + SqlTableName;
		public const string SqlInsertCommand = "INSERT INTO " + SqlTableName + " (Name , Remark) VALUES (@Name , @Remark) ";
		public const string SqlUpdateCommand = "UPDATE " + SqlTableName + " SET Name=@Name , Remark=@Remark WHERE ID=@ID";
		public const string SqlDeleteCommand = "DELETE FROM " + SqlTableName + " WHERE ID=@ID";
